...
후속 운영 점검에서 실패 이력이 있는 녹음의 상세 조회가 API의 모델 캐시 검사 때문에
500으로 실패하는 회귀를 확인했다. 아래의 기존 GPU 검증 결과는 보존하되,
선행 PR #56~#59는 2026-09-10에 모두 병합되었다. 이 회귀는 최신 main에서
별도 수정 PR로 처리하며 새 수정의 실제 GPU 검증은 기존 결과와 구분한다.
확인 근거와 수정 계획은 [105 상세 조회 500 수정 계획](105_r9_detail_500_fix_plan.md)을 따른다.
...
당시 기록 PR #59는 이후 병합되었다. 새 수정의 진행 상태는
[106 API 복구 검사 분리](106_r9_api_recovery_inspection.md)를 따른다.
