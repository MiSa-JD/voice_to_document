...
요약 요청의 네트워크 timeout은 `SUMMARY_REQUEST_TIMEOUT_SECONDS`로 조절합니다(기본 300초,
유한한 양수). 장문 운영 검증을 위한 초기값이며 전체 작업의 시간 제한이 아닙니다.
교정·chunk 요청마다 각각 적용되며, 값을 바꿔도 기존 요약은 stale 처리되지 않습니다.

공개 합성 45분 회의(약 3만~5만 자)는 별도 옵션으로 실제 API 평가할 수 있습니다.
`docker compose --profile summary-eval run --build --rm summary-eval python -m app.summary_eval --long`
을 실행하면 앞·중간·끝 필수 사실과 근거 보존, 안전한 실패 사유 및 소요 시간을 출력합니다.
API 비용이 발생하며 기본 `make summary-eval`의 5개 범주 평가는 유지됩니다.
요약 timeout과 일시 오류는 최대 세 번 시도하며 마지막 실패 때만 recording이 실패합니다.
최종 실패 후에는 화면의 요약 재요청으로 복구할 수 있습니다.
